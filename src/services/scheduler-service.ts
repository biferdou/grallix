...
export class SchedulerService {
  ...
  private scheduleDailyStandups(): void {
    ...
    cron.schedule(config.scheduler.dailyStandup, async () => {
          ...
          if (channel) {
            ...
            channel.send({ embeds: [standupEmbed] }).then((message) => {
              ...
              collector.on("end", async () => {
                // Save standup data
                const standups = await this.db.readStandups();
                standups.standups.push({
                  channelId,
                  date: new Date().toISOString(),
                  responses,
                });
                await this.db.writeStandups(standups);
  ...
  private scheduleWeeklySummaries(): void {
    ...
    cron.schedule(config.scheduler.weeklySummary, async () => {
      const settings = await this.db.readSettings();
